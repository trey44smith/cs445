...
void semanticPrintTree(TreeNode *tree, int indent, int sibling)
{
            ...
            case ParamK:
                switch(tree->expType)
                {
                    case Integer:
                        if(tree->isArray)
                        {
                            printf("Parm: %s is array of type int [line: %d]\n", tree->attr.name, tree->lineno);
                        }
                        else
                        {
                            printf("Parm: %s of type int [line: %d]\n", tree->attr.name, tree->lineno);
                    ...
                    case boolean:
                        if(tree->isArray)
                        {
                            printf("Parm: %s is array of type bool [line: %d]\n", tree->attr.name, tree->lineno);
                        }
                        else
                        {
                            printf("Parm: %s of type bool [line: %d]\n", tree->attr.name, tree->lineno);
                    ...
                    case Char:
                        if(tree->isArray)
                        {
                            printf("Parm: %s is array of type char [line: %d]\n", tree->attr.name, tree->lineno);
                        }
                        else
                        {
                            printf("Parm: %s of type char [line: %d]\n", tree->attr.name, tree->lineno);
    ...
    else if(tree->nodekind == ExpK)
    {
        ...
        {
            case OpK:
                printf("Op: %s of type ", tree->attr.name); getExpType(tree->expType); printf(" [line: %d]\n", tree->lineno);
                break;
            case ConstantK:
                switch (tree->expType)
                {
                    case Char:
                        ...
                        break;
                    default:
                        printf("Const %s of type ", tree->attr.name); getExpType(tree->expType); printf(" [line: %d]\n", tree->lineno);
                       // printf("check check ");
                        break;
                }
                break;
            case IdK:
                printf("Id: %s of type ", tree->attr.name); getExpType(tree->expType); printf(" [line: %d]\n", tree->lineno);
                break;
            case AssignK:
                printf("Assign: %s of type int [line: %d]\n", tree->attr.name, tree->lineno);
                break;
            case CallK:
                printf("Call: %s of type ", tree->attr.name); getExpType(tree->expType); printf(" [line: %d]\n", tree->lineno);
                break;
            default:
                printf("error\n");
                break;
